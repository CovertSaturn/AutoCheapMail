...
run_all.py: An orchestrator script that ties the entire process together in a single automated flow. It changes the standard execution order to ensure the droplet's hostname perfectly matches the mail server's Fully Qualified Domain Name (FQDN) for proper reverse DNS (PTR) alignment.
...
Add the DKIM Record: The deploy script will generate a DKIM key. Retrieve the value by running:
...
Open Port 25: DigitalOcean blocks outbound port 25 by default to prevent spam. You must open a support ticket with DigitalOcean requesting them to unblock outbound port 25 for your droplet so you can send emails to external providers (receiving works immediately).

Managing Mailboxes: You can add more mailboxes at any time by running the setup utility directly on your VPS:
